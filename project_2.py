...
class Node:
    # Bổ sung thêm h và f vào cấu trúc Node
    def __init__(self, state, parent=None, move=None, cost=0, h=0):
        self.state = state
        self.parent = parent
        self.move = move
        self.cost = cost # g(n): Chi phí thực tế (số bước đi)
        self.h = h       # h(n): Heuristic (số rác 'X' còn lại)
        self.f = self.cost + self.h # f(n) = g(n) + h(n)
...
class VacuumAIApp:
    ...
    def setup_ui(self):
        left_frame = tk.Frame(self.root, width=170, bg="lightgray", relief=tk.RAISED, borderwidth=2)
        left_frame.pack(side=tk.LEFT, fill=tk.Y)
        tk.Label(left_frame, text="Algo", font=("Arial", 16, "bold"), bg="lightgray").pack(pady=10)
        
        # Thêm Greedy và A* vào danh sách
        ...
        for text, val in algos:
            tk.Radiobutton(left_frame, text=text, variable=self.algo_choice, value=val, 
                           ...
                           width=14, height=2, selectcolor="lightblue").pack(pady=5, padx=10)
    ...
    def count_dirt(self, state):
        """Hàm Heuristic h(n): đếm số lượng rác 'X' còn trên bản đồ"""
    ...
    def run_algo(self):
        ...
        if algo == "BFS_1":
            ...
            goal_node = self.dls(start_node, limit=15)
        elif algo == "UCS":
            goal_node = self.ucs(start_node)
        elif algo == "GREEDY":
            goal_node = self.solve_greedy(start_node)
        else:
            goal_node = self.solve_astar(start_node)
        ...
        while len(frontier) > 0:
            ...
            for child in self.get_successors(current_node):
                ...
                if state_str not in reached:
                    ...
                    frontier.append(child)
                    self.log(f"  Thêm vào Frontier: {child.move} (h={child.h})")
        return None
    ...
    def solve_astar(self, start_node):
        ...
        frontier = [start_node]
        # Lưu trữ trạng thái cùng giá trị f(n) tốt nhất đã tìm thấy
        reached = {str(start_node.state): start_node.f}

        while len(frontier) > 0:
            # Ưu tiên f(n), nếu f bằng nhau thì ưu tiên h(n) 
            frontier.sort(key=lambda n: (n.f, n.h))
            current_node = frontier.pop(0)

            self.log(f"Xét Node (g={current_node.cost}, h={current_node.h}, f={current_node.f}): {current_node.move if current_node.move else 'Start'}")
            ...
            for child in self.get_successors(current_node):
                ...
                state_str = str(child.state)
                # Nếu trạng thái chưa được xét, hoặc tìm được đường đi có f(n) nhỏ hơn [cite: 72]
                if state_str not in reached or child.f < reached[state_str]:
                    reached[state_str] = child.f
                    frontier.append(child)
                    self.log(f"  Thêm vào Frontier: {child.move} (f={child.f})")
