...
class VacuumAIApp:
    ...
    def setup_ui(self):
        left_frame = tk.Frame(self.root, width=160, bg="lightgray", relief=tk.RAISED, borderwidth=2)
    ...
    def dls(self, start_node, limit=15):
        frontier = [start_node]
        # Sử dụng dict cho reached để lưu depth, tránh bỏ sót đường đi ngắn hơn đến cùng 1 state
        ...
        while frontier:
            ...
            if node.cost >= limit:
                self.log("  => Cutoff (Chạm giới hạn độ sâu)")
                continue
                
            for child in self.get_successors(node):
                if child not in reached or reached[child] > child.cost:
    ...
    def ucs(self, start_node):
        ...
        while frontier:
            # Sort frontier theo "Cost = số ô sai hiện tại (số chữ X)" tăng dần
            frontier.sort(key=lambda n: sum(row.count('X') for row in n.state))
            node = frontier.pop(0) # Pop node có cost thấp nhất ở đầu
            
            current_cost = sum(row.count('X') for row in node.state)
            self.log(f"Xét Node (Cost rác: {current_cost}): {node.move if node.move else 'Start'}")
            ...
            for child in self.get_successors(node):
                if child not in reached:
                    reached.add(child)
                    frontier.append(child)
                    self.log(f"  Thêm vào Frontier: {child.move} (Cost rác: {sum(row.count('X') for row in child.state)})")
        return None
