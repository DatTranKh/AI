...
class VacuumAIApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Vacuum Cleaner AI System - Complete 13 Algos")
    ...
    def setup_ui(self):
        left_frame = tk.Frame(self.root, width=220, bg="lightgray", relief=tk.RAISED, borderwidth=2)
        ...
        for text, val in algos:
            tk.Radiobutton(left_frame, text=text, variable=self.algo_choice, value=val, 
                           ...
                           width=20, height=1, selectcolor="lightblue").pack(pady=3, padx=10)
    ...
    def generate_random_state(self, num_x):
        """Tạo một trạng thái ngẫu nhiên chứa đúng 1 chữ O và số lượng chữ X tương ứng"""
    ...
    def run_algo(self):
        ...
        if goal_node:
            ...
            if not self.is_goal(goal_node.state):
                self.set_console(f"Thất bại (Hết lượt restart)! Đường đi cuối: {' -> '.join(moves)}", error=True)
            else:
    ...
    def random_restart_hill_climbing(self, start_node):
        num_x = self.count_dirt(start_node.state) # Lưu lại số lượng chữ X ban đầu để sinh ngẫu nhiên
        max_restart = 10
        
        for i in range(1, max_restart + 1):
            self.log(f"\n=== LƯỢT CHẠY THỨ {i}/{max_restart} ===")
            
            # Lượt 1: dùng trạng thái do người dùng thiết lập
            if i == 1:
                current_state = copy.deepcopy(start_node.state)
                current = Node(current_state)
            # Các lượt sau: Khởi tạo lại một bản đồ ngẫu nhiên mới
            else:
                current_state = self.generate_random_state(num_x)
                current = Node(current_state)
                self.log("-> Khởi tạo lại bản đồ ngẫu nhiên mới!")

            while True:
                if self.is_goal(current.state):
                    self.log(f"-> Tìm thấy GOAL ở lượt thứ {i}!")
                    return current
                
                neighbors = self.get_successors(current)
                if not neighbors:
                    self.log("  -> Không có nước đi lân cận hợp lệ.")
                    break
                
                # Lọc ra tập Better_Neighbors có Value > Value(Current)
                better_neighbors = [n for n in neighbors if self.get_value(n.state) > self.get_value(current.state)]
                
                if not better_neighbors: # NẾU Better_Neighbors RỖNG
                    self.log("  -> Bị kẹt ở cực đại cục bộ! Thoát vòng lặp để chuyển sang lượt restart tiếp theo.")
                    break # Thoát vòng lặp TRONG KHI để thực hiện lượt i tiếp theo
                else:
                    ...
                    current = best_neighbor
                    self.log(f"  -> Chọn Next_State tốt nhất: {current.move} (Value={self.get_value(current.state)})")
                    
        self.log("\n-> Thất bại: Chạy hết sạch 10 lượt restart mà không chạm được Goal")
        return current # Trả về trạng thái kẹt cuối cùng nếu thất bại
